...
(function ($) {
	...
	if ($('.footer-big-text').length > 0) {
		...
		let endGradient = chroma.scale(['#000000', '#000000', '#000000', '#000000', '#000000']);
		cta.to(chars, {
			duration: 0.5,
			scaleY: 0.6,
			ease: "power1.out",
	...
	if ($('.cta-text').length > 0) {
		...
		let chars = mySplitText.chars;
		let endGradient = chroma.scale(['#99ba4a', '#7ca124ff', '#678815ff', '#5f7a20ff', '#99ba4a']);
		cta.to(chars, {
			duration: 0.5,
			scaleY: 0.6,
			ease: "power1.out",
...
// Animate Latest Projects section heading
if ($('.tp-project-3-area .tp_reveal_anim').length > 0) {
  let tl = gsap.timeline({
    scrollTrigger: {
      trigger: '.tp-project-3-area',
      start: 'top 80%', // adjust when it starts
      once: true
    }
  });

  gsap.set('.tp-project-3-area .tp_reveal_anim', { opacity: 0 });
  tl.to('.tp-project-3-area .tp_reveal_anim', {
    opacity: 1,
    duration: 0.8,
    ease: 'power1.out'
  });

  let split = new SplitText('.tp-project-3-area .tp_reveal_anim', { type: 'chars, words' });
